...
# ─── Голодание: чтение/запись счётчика в habits DB ───

_fasting_page_id: str | None = None


def _find_fasting_page() -> str | None:
    global _fasting_page_id
    if _fasting_page_id:
        return _fasting_page_id
    ...
    try:
        ...
        print(f"[HEALTH_NOTION] {datetime.now().strftime('%H:%M:%S')} ❌ Не удалось найти страницу Голодания.")
        return None

    for page in data.get("results", []):
        props = page.get("properties", {})
        title = _get_title(props, name_field)
        if title.lower() == "голодание":
            _fasting_page_id = page["id"]
            return _fasting_page_id

    print(f"[HEALTH_NOTION] {datetime.now().strftime('%H:%M:%S')} ⚠️ Страница 'Голодание' не найдена в habits DB.")
    return None


def get_fasting_counter() -> int:
    page_id = _find_fasting_page()
    if not page_id:
        return 0
    ...
    try:
        ...
        return int(_get_number(props, counter_field))
    except Exception:
        return 0


def set_fasting_counter(value: int) -> bool:
    page_id = _find_fasting_page()
    if not page_id:
        return False
    ...
    try:
        response = requests.patch(
            ...
            headers=headers,
            json={"properties": {counter_field: {"number": value}}},
            timeout=10,
        )
        response.raise_for_status()
        return True
    except Exception:
        print(f"[HEALTH_NOTION] {datetime.now().strftime('%H:%M:%S')} ❌ Ошибка записи счётчика голодания.")
        return False
