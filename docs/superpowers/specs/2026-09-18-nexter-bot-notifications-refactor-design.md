...
## Проверка
- `_extract_id` — чистая функция, проверяется скриптом/REPL на разных типах поля.
- End-to-end вручную: `GET /test/claims` и `GET /test/reviews` (с `secret`) —
  первый вызов прогревает состояние и молчит, после добавления новой страницы
  приходит короткое сообщение с ID.
