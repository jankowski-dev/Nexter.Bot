...
2. Ежедневный автоинкремент счётчиков привычек.
3. Уведомления о новых заявках и отзывах из Notion — **через вебхуки** (polling отсутствует).
...
| `health_notion.py` | Привычки и расписание дня |
| `scheduler.py` | APScheduler: напоминания и +1 к привычкам |
...
- Каждый день в 22:00 `increment_all_habit_counters()` увеличивает счётчик на 1
  для привычек из `health_config.yaml`. Уведомление об изменении не отправляется.
